...
class VendaController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        return DB::connection('tenant')->transaction(function () use ($dados, $user) {

            // valida cliente
            if (!DB::connection('tenant')->table('clientes')->find($dados['cliente_id'])) {
                abort(422, 'Cliente não encontrado');
            }

            // valida produtos
            foreach ($dados['itens'] as $item) {
                if (!DB::connection('tenant')->table('produtos')->find($item['produto_id'])) {
