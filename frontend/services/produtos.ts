...
export interface ImportacaoLinhaSucesso {
    linha: number;
    produto_id?: string;
    servico_id?: string;
    nome: string;
...
export interface ImportacaoLinhaIgnorada {
    linha: number;
    nome: string;
    motivo: string;
}
 
export interface ImportacaoResultadoGrupo {
    total_sucesso: number;
    total_erros: number;
    total_ignorados: number;
    sucesso: ImportacaoLinhaSucesso[];
    erros: ImportacaoLinhaErro[];
    ignorados: ImportacaoLinhaIgnorada[];
}
 
export interface ImportarProdutosResponse {
    message: string;
    produtos: ImportacaoResultadoGrupo;
    servicos: ImportacaoResultadoGrupo;
    modo?: string;
}
...
async importarProdutos(ficheiro: File): Promise<ImportarProdutosResponse> {
    const formData = new FormData();
    formData.append("ficheiro", ficheiro);
 
    const response = await api.post(`${API_PREFIX}/produtos/importar`, formData, {
