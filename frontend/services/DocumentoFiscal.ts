...
class DocumentoFiscalService {
  ...
  async listar(filtros: FiltrosDocumento = {}): Promise<PaginatedResponse<DocumentoFiscal>> {
    ...
    const response = await api.get<ApiResponse<PaginatedResponse<DocumentoFiscal>>>(`${this.baseUrl}?${params.toString()}`);
...
async emitir(dados: EmitirDocumentoDTO): Promise<DocumentoFiscal> {
    ...
    // ✅ Log para debug
    console.log('[DocumentoFiscalService.emitir] Dados recebidos:', {
  ...
  async emitirRecibo(dados: Omit<EmitirDocumentoDTO, "tipo_documento"> & { fatura_id: string }): Promise<DocumentoFiscal> {
   ...
   */
  async criarNotaCredito(documentoOrigemId: string, dados: CriarNotaCreditoDTO): Promise<RespostaNotaCredito> {
    // === VALIDAÇÕES FRONTEND ===

    // 1. Validar motivo
    if (!dados.motivo || dados.motivo.trim().length === 0) {
      throw new Error(
        "O motivo da Nota de Crédito é obrigatório. " + "Informe o motivo da correção (ex: devolução de mercadoria, erro de valor, etc.)"
      );
    }

    if (dados.motivo.trim().length < 10) {
      throw new Error("O motivo da Nota de Crédito deve ter pelo menos 10 caracteres. " + "Forneça uma descrição detalhada da correção.");
    }
    ...
    if (!TIPOS_ORIGEM_NC.includes(fatura.tipo_documento)) {
      throw new Error(
        `Nota de Crédito só pode ser emitida a partir de Fatura (FT) ou Fatura-Recibo (FR). ` + `Tipo atual: ${fatura.tipo_documento}`
      );
    }

    // 6. Validar se a fatura não está cancelada ou expirada
    if (fatura.estado === "cancelado") {
      throw new Error(`Não é possível emitir Nota de Crédito para a fatura cancelada: ${fatura.numero_documento}`);
    }

    if (fatura.estado === "expirado") {
      throw new Error(`Não é possível emitir Nota de Crédito para a fatura expirada: ${fatura.numero_documento}`);
    }
    ...
    const response = await api.post<ApiResponse<RespostaNotaCredito>>(`${this.baseUrl}/${documentoOrigemId}/nota-credito`, {
    ...
    if (response.data.data.nota_credito) {
      response.data.data.nota_credito = this._enriquecerDocumento(response.data.data.nota_credito);
    }
   ...
   */
  async criarNotaDebito(documentoOrigemId: string, dados: CriarNotaDebitoDTO): Promise<RespostaNotaDebito> {
    // === VALIDAÇÕES FRONTEND ===

    // 1. Validar itens
    if (!dados.itens || dados.itens.length === 0) {
      throw new Error(
        "A Nota de Débito deve conter pelo menos um item. " + "Os itens devem descrever serviços adicionais, juros ou multas."
      );
    }
    ...
    for (const item of dados.itens) {
      ...
      const isServico = item.eh_servico === true || (item.produto && item.produto.tipo === "servico");
      ...
      const descricaoLower = item.descricao.toLowerCase();
      if (
        descricaoLower.includes("juro") ||
        descricaoLower.includes("juros") ||
        descricaoLower.includes("multa") ||
        descricaoLower.includes("penalidade")
      ) {
    ...
    if (!temServico) {
      throw new Error(
        "A Nota de Débito deve conter pelo menos um serviço. " + "Os itens devem descrever serviços adicionais, juros ou multas."
      );
    }
    ...
    if (fatura.estado === "cancelado") {
      throw new Error(`Não é possível emitir Nota de Débito para a fatura cancelada: ${fatura.numero_documento}`);
    }

    if (fatura.estado === "expirado") {
      throw new Error(`Não é possível emitir Nota de Débito para a fatura expirada: ${fatura.numero_documento}`);
    }
    ...
    const response = await api.post<ApiResponse<RespostaNotaDebito>>(`${this.baseUrl}/${documentoOrigemId}/nota-debito`, {
      itens: dados.itens,
    ...
    if (response.data.data.nota_debito) {
      response.data.data.nota_debito = this._enriquecerDocumento(response.data.data.nota_debito);
    }
  ...
  async gerarRecibo(documentoId: string, dados: GerarReciboDTO): Promise<DocumentoFiscal> {
    const response = await api.post<ApiResponse<DocumentoFiscal>>(`${this.baseUrl}/${documentoId}/recibo`, dados);
    if (!response.data.success) throw new Error(response.data.message);
    return this._enriquecerDocumento(response.data.data);
  }

  async listarRecibos(documentoId: string): Promise<DocumentoFiscal[]> {
    const response = await api.get<ApiResponse<DocumentoFiscal[]>>(`${this.baseUrl}/${documentoId}/recibos`);
    if (!response.data.success) throw new Error(response.data.message);
  ...
  async vincularAdiantamento(
    ...
    const response = await api.post<ApiResponse<{ adiantamento: DocumentoFiscal; fatura: DocumentoFiscal }>>(
      `${this.baseUrl}/${adiantamentoId}/vincular-adiantamento`,
      dados
    );
    if (!response.data.success) throw new Error(response.data.message);
    return {
  ...
  async cancelar(id: string, dados: CancelarDocumentoDTO): Promise<DocumentoFiscal> {
    const response = await api.post<ApiResponse<DocumentoFiscal>>(`${this.baseUrl}/${id}/cancelar`, dados);
    if (!response.data.success) throw new Error(response.data.message);
    return this._enriquecerDocumento(response.data.data);
  }
  ...
  async getEvolucaoMensal(ano?: number): Promise<EvolucaoDados[]> {
    ...
    const response = await api.get<ApiResponse<{ ano: number; evolucao: EvolucaoDados[] }>>(`/api/dashboard/evolucao-mensal${params}`);
  ...
  async exportarExcel(filtros: FiltrosDocumento = {}): Promise<void> {
    ...
    const response = await api.get(`${this.baseUrl}/exportar-excel?${params.toString()}`, { responseType: "blob" });
  ...
  calcularValorPendente(documento: DocumentoFiscal): number {
    ...
    const totalPago = documento.recibos?.reduce((sum, r) => (r.estado !== "cancelado" ? sum + Number(r.total_liquido) : sum), 0) ?? 0;

    if (documento.tipo_documento === "FA") {
      return Math.max(0, Number(documento.total_liquido) - totalPago);
    }

    const totalAdiantamentos = documento.faturasAdiantamento?.reduce((sum, a) => sum + Number(a.pivot?.valor_utilizado ?? 0), 0) ?? 0;
  ...
  calcularValorPago(documento: DocumentoFiscal): number {
    ...
    return documento.recibos?.reduce((sum, r) => (r.estado !== "cancelado" ? sum + Number(r.total_liquido) : sum), 0) ?? 0;
  }
  ...
  calcularSaldoDisponivel(documento: DocumentoFiscal): number {
    ...
    const totalCreditado =
      documento.notasCredito?.reduce((sum, nc) => (nc.estado !== "cancelado" ? sum + Number(nc.total_liquido) : sum), 0) ?? 0;
  ...
  podeGerarRecibo(documento: DocumentoFiscal): boolean {
    ...
    return ["FT", "FA", "FP"].includes(documento.tipo_documento) && ["emitido", "parcialmente_paga"].includes(documento.estado);
  }
  ...
  private _enriquecerDocumento(doc: DocumentoFiscal): DocumentoFiscal {
    ...
    const totalCreditado =
      doc.notasCredito?.reduce((sum, nc) => (nc.estado !== "cancelado" ? sum + Number(nc.total_liquido) : sum), 0) ?? 0;
    const totalDebitos = doc.notasDebito?.reduce((sum, nd) => (nd.estado !== "cancelado" ? sum + Number(nd.total_liquido) : sum), 0) ?? 0;
    ...
    return {
      ...
      percentual_retencao:
        Number(doc.base_tributavel) > 0 ? Math.round((Number(doc.total_retencao) / Number(doc.base_tributavel)) * 100 * 100) / 100 : 0,
...
export const useCriarNotaCredito = () => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ documentoOrigemId, dados }: { documentoOrigemId: string; dados: CriarNotaCreditoDTO }) =>
...
export const useCriarNotaDebito = () => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ documentoOrigemId, dados }: { documentoOrigemId: string; dados: CriarNotaDebitoDTO }) =>
...
export const useCancelarDocumento = () => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ id, dados }: { id: string; dados: CancelarDocumentoDTO }) => documentoFiscalService.cancelar(id, dados),
    onSuccess: (_, variables) => {
