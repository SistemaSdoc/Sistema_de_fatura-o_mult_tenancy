    ...
    <div className="flex w-screen h-screen overflow-hidden" style={{ backgroundColor: colors.background }}>
      ...
      >
        ...
        <div className="flex items-center gap-3 h-16 px-4 border-b" style={{ borderColor: colors.border }}>
          ...
          {sidebarOpen && <span className="text-sm font-semibold truncate" style={{ color: colors.text }}>{nomeEmpresa}</span>}
          {isMobile && sidebarOpen && (
            <button onClick={closeSidebar} className="p-1 ml-auto transition-transform hover:scale-110 active:scale-95" style={{ color: colors.text }}>
              <X size={20} />
            </button>
          )}
        </div>
              ...
              return (
                <div key={item.label}>
                  <div
                    ...
                    style={{ backgroundColor: active ? colors.secondary : "transparent", color: active ? "white" : colors.text }}
                    onClick={(e) => handleMainItemClick(item, e)}
                  >
                    {hasLinks ? (
                      <div className="flex items-center flex-1 gap-3 min-w-0">
                        <item.icon size={19} style={{ color: active ? "white" : colors.secondary, flexShrink: 0 }} />
                        {sidebarOpen && <span className="text-sm font-medium truncate" style={{ color: active ? "white" : colors.text }}>{item.label}</span>}
                      </div>
                    ) : (
                      <Link href={item.path} className="flex items-center flex-1 gap-3 min-w-0" onClick={handleLinkClick}>
                        <item.icon size={19} style={{ color: active ? "white" : colors.secondary, flexShrink: 0 }} />
                        {sidebarOpen && <span className="text-sm font-medium truncate" style={{ color: active ? "white" : colors.text }}>{item.label}</span>}
                      </Link>
                    )}
                    {hasLinks && sidebarOpen && (
                      <div className="ml-1 transition-transform duration-250" style={{ transform: isOpen ? "rotate(180deg)" : "rotate(0deg)" }}>
                        ...
                        return (
                          <Link key={link.path} href={link.path} onClick={handleLinkClick}>
                            <div
                              ...
                              style={{ borderColor: linkActive ? colors.primary : "transparent", backgroundColor: linkActive ? `${colors.primary}15` : "transparent" }}
                            >
                              {link.icon && <link.icon size={15} style={{ color: colors.text, flexShrink: 0 }} />}
                              <span className="text-xs truncate" style={{ color: linkActive ? colors.text : colors.textSecondary, fontWeight: linkActive ? "600" : "400" }}>{link.label}</span>
                            </div>
                          </Link>
                        );
                      })}
        ...
        <div className="p-2 border-t" style={{ borderColor: colors.border }}>
          <div onClick={abrirModalLogout} className="flex items-center gap-3 px-3 py-2.5 cursor-pointer transition-all hover:translate-x-1 active:scale-98">
            <LogOut size={19} className="text-red-500" />
            {sidebarOpen && <span className="text-sm font-medium" style={{ color: colors.text }}>Sair</span>}
          </div>
        </div>
      </aside>
      ...
      <div className="flex flex-col flex-1 w-full overflow-hidden">
        <header className="flex items-center justify-between gap-3 px-3 h-14 border-b shadow-sm md:h-16 md:px-6" style={{ backgroundColor: colors.card, borderColor: colors.border }}>
          <div className="flex items-center gap-2 min-w-0">
            {isMobile && (
              <button onClick={() => setSidebarOpen(true)} className="p-2 transition-transform" style={{ color: colors.primary }}>
                <Menu size={20} />
              </button>
            )}
            <h1 className="text-sm font-bold truncate md:text-base" style={{ color: colors.text }}>{currentPageLabel}</h1>
          </div>
            ...
            {(userRole === "admin" || userRole === "operador") && (
              <div className="relative">
                ...
                >
                  ...
                  {totalNotificacoes > 0 && (
                    <span className="absolute -top-1 -right-1 text-[10px] font-bold flex items-center justify-center px-1 text-white animate-pulse" style={{ backgroundColor: produtosSemEstoque.length > 0 ? colors.danger : "#f59e0b" }}>
                ...
                {notificacoesAberto && (
                  <>
                    <div className={`absolute right-0 z-50 mt-2 overflow-hidden border shadow-lg transition-all duration-200 ${panelAnimating ? "opacity-0 scale-95" : "opacity-100 scale-100"}`} style={{ backgroundColor: colors.card, borderColor: colors.border, width: "min(380px, calc(100vw - 20px))", transformOrigin: "top right" }}>
                      <div className="p-3 border-b" style={{ borderColor: colors.border }}>
                        <div className="flex items-center justify-between">
                          <h3 className="text-sm font-semibold" style={{ color: colors.text }}>Alertas</h3>
                          <button onClick={() => setNotificacoesAberto(false)} className="p-1 transition-transform hover:scale-110 active:scale-95"><X size={16} style={{ color: colors.textSecondary }} /></button>
                        </div>
                        {ultimaAtualizacao && <p className="text-xs mt-1" style={{ color: colors.textSecondary }}>{ultimaAtualizacao.toLocaleTimeString("pt-PT")}</p>}
                      </div>
                      <div className="flex border-b" style={{ borderColor: colors.border }}>
                        <button onClick={() => setAbaAtiva("baixo")} className="flex-1 py-2 px-3 text-xs font-medium transition-all hover:scale-105" style={{ color: abaAtiva === "baixo" ? colors.text : colors.textSecondary, borderBottom: abaAtiva === "baixo" ? `2px solid ${colors.secondary}` : "none" }}>Baixo ({produtosEstoqueBaixo.length})</button>
                        <button onClick={() => setAbaAtiva("zero")} className="flex-1 py-2 px-3 text-xs font-medium transition-all hover:scale-105" style={{ color: abaAtiva === "zero" ? colors.text : colors.textSecondary, borderBottom: abaAtiva === "zero" ? `2px solid ${colors.secondary}` : "none" }}>Zero ({produtosSemEstoque.length})</button>
                      </div>
                      <div className="max-h-80 overflow-y-auto">
                        {loadingNotificacoes ? (
                          <div className="flex items-center justify-center py-6"><Loader2 className="w-4 h-4 animate-spin" style={{ color: colors.primary }} /></div>
                          ...
                          <div style={{ borderColor: colors.border }} className="divide-y">
                            {produtosEstoqueBaixo.map((produto, idx) => (
                              <div key={produto.id} className="p-3 transition-all hover:translate-x-1" style={{ animation: `fadeIn 0.2s ease-out ${idx * 0.03}s forwards` }}>
                                <div className="flex gap-2">
                                  <div className="p-1.5" style={{ backgroundColor: colors.warning }}><TrendingDown size={14} className="text-white" /></div>
                                  <div className="min-w-0 flex-1"><p className="text-xs font-medium truncate" style={{ color: colors.text }}>{produto.nome}</p><p className="text-xs mt-0.5" style={{ color: colors.textSecondary }}>{produto.estoque_atual} / {produto.estoque_minimo}</p></div>
                                </div>
                              </div>
                            ))}
                          </div>
                          ...
                          <div style={{ borderColor: colors.border }} className="divide-y">
                            {produtosSemEstoque.map((produto, idx) => (
                              <div key={produto.id} className="p-3 transition-all hover:translate-x-1" style={{ animation: `fadeIn 0.2s ease-out ${idx * 0.03}s forwards` }}>
                                <div className="flex gap-2"><div className="p-1.5" style={{ backgroundColor: colors.danger }}><AlertCircle size={14} className="text-white" /></div><p className="text-xs font-medium truncate" style={{ color: colors.text }}>{produto.nome}</p></div>
                              </div>
                            ))}
                          </div>
                        ) : (
                          <div className="py-8 text-center"><p className="text-xs" style={{ color: colors.textSecondary }}>Sem alertas</p></div>
                        )}
                      </div>
                      <div className="p-2 border-t text-center" style={{ borderColor: colors.border }}>
                        <button onClick={buscarNotificacoesEstoque} disabled={loadingNotificacoes} className="text-xs font-medium transition-all hover:scale-105 disabled:opacity-50" style={{ color: colors.text }}>{loadingNotificacoes ? "Atualizando..." : "Atualizar"}</button>
                      </div>
                    </div>
      ...
      {logoutModalOpen && (
        <div className={`fixed inset-0 z-50 flex items-center justify-center p-4 transition-all duration-200 ${modalAnimating ? "opacity-0" : "opacity-100"}`} style={{ backgroundColor: "rgba(0, 0, 0, 0.5)" }} onClick={fecharModalLogout}>
          <div className={`w-full max-w-sm overflow-hidden shadow-lg transition-all duration-200 ${modalAnimating ? "scale-95 opacity-0" : "scale-100 opacity-100"}`} style={{ backgroundColor: colors.card }} onClick={(e) => e.stopPropagation()}>
            <div className="p-4 border-b text-center md:p-6 md:pb-4" style={{ borderColor: colors.border }}><h2 className="text-lg font-bold md:text-xl" style={{ color: colors.text }}>Confirmar</h2><p className="text-xs mt-1 md:text-sm" style={{ color: colors.textSecondary }}>Deseja sair da sua conta?</p></div>
            {logoutError && <div className="p-2 mx-4 mb-3 text-xs text-center animate-shake" style={{ backgroundColor: theme === "dark" ? "#442200" : "#fef3c7", border: `1px solid ${theme === "dark" ? "#854d0e" : "#fbbf24"}`, color: theme === "dark" ? "#fbbf24" : "#92400e" }}>{logoutError}</div>}
            <div className="p-3 border-b md:p-4" style={{ backgroundColor: theme === "dark" ? "#1a1a1a" : "#f9fafb", borderColor: colors.border }}>
              <div className="flex items-center gap-2"><div className="flex items-center justify-center w-7 h-7 text-xs font-bold text-white" style={{ background: `linear-gradient(135deg, ${colors.secondary} 0%, ${colors.primary} 100%)` }}>{userInitial}</div><div className="min-w-0"><p className="text-xs font-medium truncate md:text-sm" style={{ color: colors.text }}>{userName}</p><p className="text-[10px] truncate md:text-xs" style={{ color: colors.textSecondary }}>{userEmail}</p></div></div>
            </div>
            <div className="flex gap-2 p-3 md:gap-3 md:p-4">
              <button onClick={fecharModalLogout} disabled={logoutLoading} className="flex-1 py-2 px-3 text-xs font-medium transition-all hover:scale-105 active:scale-95 md:text-sm" style={{ backgroundColor: theme === "dark" ? "#333" : "#e5e7eb", color: colors.text }}>Cancelar</button>
              <button onClick={handleLogout} disabled={logoutLoading} className="flex-1 py-2 px-3 text-xs font-medium text-white transition-all flex items-center justify-center gap-1 hover:scale-105 active:scale-95 md:text-sm" style={{ backgroundColor: colors.danger }}>{logoutLoading ? <Loader2 className="w-3 h-3 md:w-4 md:h-4 animate-spin" /> : <><LogOut size={14} /><span className="hidden sm:inline">Sair</span></>}</button>
            </div>
          </div>
        </div>
      )}
