...
const LogoUploader = ({
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      ...
      const response = await api.post<{ success: boolean; logo_url: string }>("/api/empresa/logo", formData, {
    ...
    <div className="space-y-3">
      ...
      <div
        ...
        {preview ? (
          ...
          <div className="w-16 h-16 rounded-full flex items-center justify-center" style={{ backgroundColor: colors.hover }}>
    ...
    <div className="space-y-6">
      ...
      <Card style={{ backgroundColor: colors.card, borderColor: colors.border }}>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
            {empresa?.subdomain && <ReadonlyField label="Subdomínio" colors={colors} icon={Globe} value={empresa.subdomain} />}
            {empresa?.created_at && (
              <ReadonlyField label="Data de registro" colors={colors} value={new Date(empresa.created_at).toLocaleDateString("pt-PT")} />
            )}
          </div>
        </CardContent>
      </Card>
      ...
      <Card style={{ backgroundColor: colors.card, borderColor: colors.border }}>
        <CardHeader>
          ...
          <CardDescription style={{ color: colors.textSecondary }}>Aparece em documentos fiscais e no sistema</CardDescription>
        </CardHeader>
        <CardContent>
          <LogoUploader
            colors={colors}
      ...
      <Card style={{ backgroundColor: colors.card, borderColor: colors.border }}>
        <CardHeader>
          ...
          <CardDescription style={{ color: colors.textSecondary }}>Informações principais</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <FormInput
              ...
              icon={Building2}
            />
            <FormInput label="NIF" name="nif" value={form.nif} onChange={handleChange} colors={colors} disabled={isSuspended} icon={Hash} />
            <FormInput
              label="E-mail"
              name="email"
              type="email"
            ...
            <div className="space-y-2 md:col-span-2">
              <Label htmlFor="endereco" style={{ color: colors.text }} className="flex items-center gap-1.5">
      ...
      <Card style={{ backgroundColor: colors.card, borderColor: colors.border }}>
        <CardHeader>
          ...
          <CardDescription style={{ color: colors.textSecondary }}>Padrões gerais</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              ...
              <div
                className="flex items-center gap-3 h-10 px-3 border rounded-md"
                style={{
                  borderColor: colors.border,
                  backgroundColor: isSuspended ? colors.hover : colors.card,
                }}>
                <span className="text-sm" style={{ color: colors.textSecondary }}>
                  {form.regime_fiscal === "simplificado" ? "0% - Regime simplificado não liquida IVA" : "Taxa definida nas categorias"}
                </span>
              </div>
            </div>
          </div>
          {form.regime_fiscal === "simplificado" && (
            <div
              ...
              Regime simplificado: os documentos e vendas não liquidam IVA. No regime geral, a taxa aplicada vem da categoria do produto ou
              da taxa configurada no serviço.
            </div>
          )}
        </CardContent>
        <CardFooter className="flex justify-end border-t pt-6" style={{ borderColor: colors.border }}>
      ...
      <Card style={{ backgroundColor: colors.card, borderColor: colors.border }}>
        <CardHeader>
          ...
          <CardDescription style={{ color: colors.textSecondary }}>Informações para pagamentos e transferências</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <FormInput
      ...
      <Dialog open={showConfirmDialog} onOpenChange={setShowConfirmDialog}>
        <DialogContent style={{ backgroundColor: colors.card, borderColor: colors.border }}>
          <DialogHeader>
            <DialogTitle style={{ color: isSuspended ? colors.primary : colors.secondary }}>
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={togglingStatus}
              style={{
                backgroundColor: isSuspended ? colors.success : colors.secondary,
