...
export default function RegisterCompanyPage() {
    ...
    // ✅ Função para obter CSRF token fresco
    const refreshCsrf = async () => {
        try {
            await api.get("/sanctum/csrf-cookie");
            await new Promise((resolve) => setTimeout(resolve, 100));
            console.log("[CSRF] Token atualizado com sucesso");
        } catch (error) {
    ...
    const uploadLogo = async (file: File): Promise<string> => {
        ...
        try {
            ...
            // Se for erro 419, tenta novamente uma vez
            if (error.response?.status === 419) {
                console.log("CSRF expirado, tentando novamente...");
                await refreshCsrf();
                const retryResponse = await api.post(
                    "/api/upload-temp-logo",
                    uploadFormData,
                    {
                        headers: { "Content-Type": "multipart/form-data" },
                    },
                ...
                if (retryResponse.data.success) {
                    return retryResponse.data.logo_url;
                }
            }
            throw new Error(
                error.response?.data?.message || "Erro ao fazer upload do logo",
            );
        }
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            // ✅ Garantir CSRF antes de toda a operação
            await refreshCsrf();
            ...
            <div className="max-w-4xl mx-auto">
                ...
                >
                    <div className="flex-1 py-3 text-center">
                        <div
                            className={`inline-flex items-center gap-2 text-sm font-medium ${step === 1 ? "opacity-100" : "opacity-50"}`}
                    ...
                    <div className="flex-1 py-3 text-center">
                        <div
                            className={`inline-flex items-center gap-2 text-sm font-medium ${step === 2 ? "opacity-100" : "opacity-50"}`}
                ...
                >
                    <form onSubmit={handleSubmit}>
                        <div className="p-8">
                            ...
                            {error && (
                                <div
                                    className="flex items-center gap-3 p-4 mb-6 border-l-4"
                                    style={{
                                    ...
                                    <div>
                                        ...
                                        <div className="flex items-center gap-4">
                                            {logoPreview ? (
                                                ...
                                                <label
                                                    className="flex flex-col items-center justify-center w-16 h-16 border-2 border-dashed cursor-pointer hover:border-primary"
                                                    style={{ borderColor: colors.border }}
                                                >
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                        ...
                                        <div className="relative">
                                            ...
                                            <select
                                                ...
                                                onChange={handleChange}
                                                className="w-full pl-10 pr-4 py-3 border outline-none text-sm "
                                                style={{
                        ...
                        >
                            <Link
                                href="/login"
                                className="group inline-flex items-center gap-2 transition-colors font-medium text-sm"
                                style={{ color: colors.secondary }}
                            >
                                <UserPlus size={18} />
                                Já tenho conta
                            ...
                            <div className="flex gap-4">
                                {step === 2 && (
                                    <button
                                        ...
                                        className="flex items-center gap-2 px-6 py-2.5 text-sm font-medium transition-opacity hover:opacity-70 "
                                        style={{ color: colors.textSecondary }}
                                    >
                                ...
                                {step === 1 ? (
                                    <button
                                        ...
                                        className="flex items-center gap-2 px-6 py-2.5 text-sm font-medium text-white transition-opacity hover:opacity-80"
                                    ...
                                    <button
                                        ...
                                        className="flex items-center gap-2 px-6 py-2.5 text-sm font-medium text-white transition-opacity hover:opacity-80 disabled:opacity-50"
                                        style={{ backgroundColor: colors.primary }}
                                    >
