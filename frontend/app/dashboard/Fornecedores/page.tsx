...
function Modal({ isOpen, onClose, title, children }: { isOpen: boolean; onClose: () => void; title: string; children: React.ReactNode }) {
  ...
  return (
    <Dialog
      ...
      <DialogContent className="sm:max-w-2xl p-0" style={{ backgroundColor: colors.card, borderColor: colors.border }}>
        ...
        <div className="p-4 overflow-y-auto max-h-[calc(90vh-68px)]">{children}</div>
      </DialogContent>
    </Dialog>
  );
}
        ...
        {(["nacional", "internacional"] as const).map((t) => {
          ...
          return (
            <label
              ...
              <input type="radio" name="tipo" value={t} checked={active} onChange={() => setField("tipo", t)} className="hidden" />
              {t === "nacional" ? (
                <Building2 className="w-4 h-4" style={{ color: active ? clr : colors.textSecondary }} />
              ) : (
                <Globe className="w-4 h-4" style={{ color: active ? clr : colors.textSecondary }} />
              )}
...
export default function FornecedoresPage() {
  ...
  const abrirArquivar = (f: Fornecedor) => {
    setSelecao(f);
    setModalArquivar(true);
  };
  const abrirRestaurar = (f: Fornecedor) => {
    setSelecao(f);
    setModalRestaurar(true);
  };
  const abrirExcluir = (f: Fornecedor) => {
    setSelecao(f);
  ...
  const fecharArquivar = () => {
    setModalArquivar(false);
    setSelecao(null);
  };
  const fecharRestaurar = () => {
  ...
  const listaVazia = abaAtiva === "ativos" ? fornecedores.length === 0 : fornecedoresDeletados.length === 0;
  ...
  return (
    <MainEmpresa>
      <div className="space-y-4 max-w-7xl mx-auto pb-6 transition-colors duration-300" style={{ backgroundColor: colors.background }}>
            ...
            <div className="flex border" style={{ borderColor: colors.border, backgroundColor: colors.card }}>
              {(["ativos", "lixeira"] as const).map((aba) => (
                <button
                  ...
                  {aba === "ativos" ? <Truck className="w-4 h-4" /> : <Trash2 className="w-4 h-4" />}
                  {aba === "ativos" ? `Ativos (${fornecedores.length})` : `Lixeira (${fornecedoresDeletados.length})`}
                </button>
              ))}
            </div>
        ...
        {loading ? (
          ...
          <div className="border text-center py-14" style={{ backgroundColor: colors.card, borderColor: colors.border }}>
            <Truck className="w-14 h-14 mx-auto mb-3" style={{ color: colors.border }} />
            <p className="text-sm mb-4" style={{ color: colors.textSecondary }}>
              {abaAtiva === "ativos" ? "Nenhum fornecedor encontrado." : "Nenhum fornecedor na lixeira."}
            </p>
            {abaAtiva === "ativos" && (
              <button onClick={abrirCriar} className="px-4 py-2 text-white text-sm" style={{ backgroundColor: colors.primary }}>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr style={{ backgroundColor: colors.primary }}>
                    {["Fornecedor", "Tipo", "Status", "Contacto", "NIF/BI", ""].map((h, i) => (
                      <th
                        key={h}
                  ...
                  {fornecedoresFiltrados.map((f) => {
                    ...
                    return (
                      <tr
                        ...
                        style={{
                          backgroundColor: f.status === "inativo" ? `${colors.hover}80` : "transparent",
                        }}
                        onMouseEnter={(e) => (e.currentTarget.style.backgroundColor = colors.hover)}
                        onMouseLeave={(e) =>
                          (e.currentTarget.style.backgroundColor = f.status === "inativo" ? `${colors.hover}80` : "transparent")
                        }>
                        <td className="py-3 px-5">
                          <div className="flex items-center gap-3">
                            <div
                              ...
                              style={{
                                backgroundColor: f.tipo === "internacional" ? `${colors.secondary}20` : colors.hover,
                              }}>
                              {f.tipo === "internacional" ? (
                                <Globe className="w-4 h-4" style={{ color: colors.secondary }} />
                              ) : (
                                <Building2 className="w-4 h-4" style={{ color: colors.textSecondary }} />
                              )}
                            </div>
                            <div className="min-w-0">
                              ...
                              {f.email && (
                                <div className="text-xs truncate max-w-[160px]" style={{ color: colors.textSecondary }}>
                                  {f.email}
                                </div>
                              )}
                              {f.deleted_at && (
                        ...
                        <td className="py-3 px-5">
                          <span
                            ...
                            style={{
                              backgroundColor: f.tipo === "internacional" ? `${colors.secondary}` : `${colors.primary}`,
                              color: f.tipo === "internacional" ? colors.text : colors.text,
                            }}>
                            {f.tipo === "internacional" ? <Globe className="w-3 h-3" /> : <Building2 className="w-3 h-3" />}
                            {getTipoLabel(f.tipo)}
                          </span>
                        </td>
                        ...
                        <td className="py-3 px-5">
                          <span
                            className="inline-flex items-center gap-1 px-2.5 py-1 text-xs font-medium"
                            style={{
                              backgroundColor: f.status === "ativo" ? `${colors.secondary}18` : `${colors.textSecondary}18`,
                              color: f.status === "ativo" ? colors.secondary : colors.textSecondary,
                            }}>
                            {f.status === "ativo" ? <CheckCircle className="w-3 h-3" /> : <XCircle className="w-3 h-3" />}
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              {nifValidacao?.valido && nifValidacao.tipo && (
                                <span
                                  ...
                                  style={{
                                    backgroundColor: nifValidacao.tipo === "NIF" ? `${colors.primary}` : `${colors.secondary}`,
                                    color: nifValidacao.tipo === "NIF" ? colors.textSecondary : colors.textSecondary,
                                  }}>
                                  {nifValidacao.tipo}
                                </span>
                              )}
      ...
      {/* Modals */}
      <Modal isOpen={modalForm} onClose={fecharForm} title={selecao ? "Editar Fornecedor" : "Novo Fornecedor"}>
        <FormFornecedor fornecedor={selecao} onSubmit={handleSubmit} onCancel={fecharForm} loading={loadingAcao} />
      </Modal>
