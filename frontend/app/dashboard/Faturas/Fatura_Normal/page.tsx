  ...
  return (
    <>
      ...
      <div className="flex items-center justify-between gap-2 py-0.5">
        ...
        <span
          className={`text-sm ${negrito ? "font-bold" : "font-medium"} tabular-nums`}
          style={{
            color: cor || (negrito ? colors.text : colors.textSecondary),
          }}
        >
          {valor}
        </span>
...
export default function NovaFaturaNormalPage() {
      ...
      try {
        const [clientesData, produtosData] = await Promise.all([
          ...
          produtoService
            ...
            .then((res) => (Array.isArray(res.produtos) ? res.produtos : [])),
        ]);
        ...
        const disponiveis = produtosFisicos.filter(
          (p) => p.estoque_atual > ESTOQUE_MINIMO,
        );
        const estoqueBaixo = produtosFisicos.filter(
  ...
  const handleProdutoChange = (produtoId: string) => {
    ...
    setFormItem((prev) => ({
      ...
      quantidade: produto
        ? isServico(produto)
          ? 1
          : Math.min(1, produto.estoque_atual)
        : 1,
      desconto: 0,
    }));
  };
  ...
  const adicionarAoCarrinho = () => {
    ...
    const itemExistenteIndex = itens.findIndex(
      (item) => item.produto_id === formItem.produto_id,
    );
    ...
    if (itemExistenteIndex >= 0) {
      ...
      if (!isServico(produto) && novaQuantidade > produto.estoque_atual) {
        setError(
          `Estoque insuficiente para ${novaQuantidade} unidades. Disponível: ${produto.estoque_atual}`,
        );
        return;
      }
      ...
      const novoItem = calcularItemCompleto(
        produto,
        formItem.quantidade,
        formItem.desconto,
      );
  ...
  const atualizarQuantidadeItem = (itemId: string, novaQuantidade: number) => {
    ...
    setItens((prev) =>
      prev.map((i, index) => (index === itemIndex ? itemAtualizado : i)),
    );
    setError(null);
  };
  ...
  const limparCarrinho = () => {
    if (
      itens.length > 0 &&
      confirm("Tem certeza que deseja limpar todos os itens?")
    ) {
      setItens([]);
    }
  };
          ...
          <table className="w-full border-collapse">
            <tbody>
              ...
              <tr className="border-b" style={{ borderColor: colors.border }}>
                <td
                  className="py-2 pl-3 pr-2 align-middle w-[100px]"
                ...
                <td className="py-2 px-2 align-middle">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <div
                      className="inline-flex rounded-md border overflow-hidden flex-shrink-0"
                      style={{ borderColor: colors.border }}
                    >
                      {(["cadastrado", "avulso"] as ModoCliente[]).map(
                        (modo) => (
                          <button
                            key={modo}
                            type="button"
                    ...
                    {modoCliente === "cadastrado" ? (
                      <select
                        className="flex-1 min-w-[150px] max-w-xs px-2.5 py-1.5 rounded-md text-sm outline-none"
                        style={inputStyles}
                      ...
                      <div className="flex flex-wrap gap-2">
                        <input
                          ...
                          className="w-40 px-2.5 py-1.5 rounded-md text-sm outline-none"
                          style={inputStyles}
                          value={clienteAvulso}
                        ...
                        <div className="flex items-center gap-2 relative">
                          ...
                          {nifError && (
                            <span
                              className="text-[10px] absolute -bottom-4 right-0"
              ...
              <tr className="border-b" style={{ borderColor: colors.border }}>
                <td
                  className="py-2 pl-3 pr-2 align-middle"
                  style={{ backgroundColor: colors.hover }}
                >
                  <div className="flex items-center gap-1.5">
                ...
                <td className="py-2 px-2 align-middle">
                  <div className="flex flex-wrap items-center gap-2">
                    <select
                      className="w-48 sm:w-56 px-2.5 py-1.5 rounded-md text-sm outline-none"
                      style={inputStyles}
                    ...
                    >
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          if (p) {
                            const maxEstoqueAtual = isServico(p)
                              ? Infinity
                              : p.estoque_atual;
                            setFormItem((prev) => ({
                              ...prev,
                    ...
                    <input
                      ...
                      className="w-20 px-2.5 py-1.5 rounded-md text-sm outline-none"
                      style={inputStyles}
              ...
              <tr>
                <td
                  className="py-2 pl-3 pr-2 align-top"
                  style={{ backgroundColor: colors.hover }}
                >
                  <div className="flex items-center gap-1.5">
                ...
                <div className="flex flex-col lg:flex-row lg:items-start gap-4">
                  <div className="flex-1 max-w-md">
                    <LinhaFiscal
                      ...
                      valor={formatarPreco(totalBase + totalDesconto)}
                      colors={colors}
                    />
                    {totalDesconto > 0 && (
                      <LinhaFiscal
                        label="Descontos"
                        valor={`−${formatarPreco(totalDesconto)}`}
                        cor={colors.danger}
                        colors={colors}
                      />
                    )}
        ...
        {itens.length > 0 && (
          <div className="flex justify-end pt-2">
            <button
              ...
              className="w-full lg:w-48 py-2.5 rounded-xl font-bold text-sm text-white shadow-lg transition-all flex items-center justify-center gap-2 disabled:opacity-40 disabled:cursor-not-allowed"
