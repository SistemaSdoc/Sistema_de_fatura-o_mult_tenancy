...
const AnimatedSection: React.FC<AnimatedSectionProps> = ({ children, animation = "fade-up", delay = 0, threshold = 0.1 }) => {
  ...
  if (animation === "fade-up") {
    animationClasses = isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10";
...
const FeatureCard: React.FC<FeatureCardProps> = ({ Icon = () => <CheckIcon color="#F9941F" />, title, description, delay = 0, colors }) => (
  <AnimatedSection animation="fade-up" delay={delay}>
    <div
      className="p-6 rounded-xl shadow-lg hover:shadow-xl transition duration-300 transform hover:scale-[1.02] border h-full"
      style={{
...
const StepCard: React.FC<StepCardProps> = ({ number, title, description, delay = 0, colors }) => (
  <AnimatedSection animation="fade-up" delay={delay}>
    <div
      className="p-6 rounded-xl shadow-lg border-t-4 h-full"
      style={{
        backgroundColor: colors.card,
        borderColor: colors.secondary,
      }}>
      <span className="text-4xl font-extrabold mb-3 block" style={{ color: colors.secondary }}>
        {number}
      </span>
...
const PricingCard: React.FC<PricingCardProps> = ({ plan, index, colors }) => {
  ...
  return (
    <AnimatedSection animation="fade-up" delay={index * 100} threshold={0.2}>
      <div
        className="flex flex-col p-6 mx-auto max-w-lg text-center rounded-xl border-2 h-full transition duration-300 ease-in-out hover:shadow-2xl hover:scale-[1.03]"
        style={{
          backgroundColor: colors.card,
          borderColor: colors.border,
        }}>
...
const processSteps = [
  {
    ...
    description: "Comece em minutos! Crie sua conta sem papelada e esteja pronto para facturar hoje mesmo.",
  },
  {
    number: 2,
    title: "Produtividade Garantida",
    description: "Otimize processos internos e dedique mais tempo ao crescimento do negócio.",
  },
...
const faqData = [
  {
    q: "O que torna o Factura Já diferente de outras plataformas?",
    a: "O FaturaJá foca-se na simplicidade e rapidez. Pode criar uma factura profissional em menos de 60 segundos, com ênfase na conformidade legal angolana e num design limpo e moderno.",
  },
  {
    q: "Posso cancelar o meu plano a qualquer momento?",
    a: "Sim, todos os planos podem ser cancelados a qualquer momento, sem taxas de rescisão. Caso cancele, mantém o acesso até ao final do ciclo de facturação.",
  },
  {
    q: "O Factura Já é compatível com telemóveis?",
    a: "Absolutamente! A plataforma é 100% responsiva, funcionando perfeitamente em dispositivos móveis, tablets e desktops.",
  },
  {
    q: "Posso adicionar vários utilizadores à minha conta?",
    a: "Sim, dependendo do plano, pode adicionar vários utilizadores com diferentes permissões para gerir clientes, produtos e facturas de forma colaborativa.",
  },
  {
    q: "Quais métodos de pagamento estão disponíveis para os clientes finais?",
    a: "Pode aceitar pagamentos por transferência bancária, Multicaixa ou cartões de débito/crédito, e acompanhar o estado das facturas (pendente, pago, cancelado).",
  },
  {
    q: "Posso gerar relatórios das minhas vendas e facturamento?",
    a: "Sim, os planos Essencial, Pro, Premium e Empresa permitem gerar relatórios detalhados de facturamento e vendas, ajudando a monitorizar o desempenho financeiro da sua empresa.",
  },
  {
    q: "As facturas cumprem a legislação fiscal angolana?",
    a: "Sim, todas as facturas emitidas pelo FaturaJá cumprem as normas da Autoridade Tributária Angolana, garantindo conformidade legal.",
  },
];
    ...
    <div className="min-h-screen font-inter transition-colors duration-300" style={{ backgroundColor: colors.background }}>
              ...
              <div className="text-center lg:text-left">
                ...
                <AnimatedSection animation="fade-up" delay={200}>
                  <p className="text-xl md:text-2xl mb-8 max-w-lg mx-auto lg:mx-0" style={{ color: colors.textSecondary }}>
                    Gere facturas profissionais em segundos, sem complicações. A ferramenta ideal para pequenos negócios e profissionais que
      ...
      <AnimatedSection animation="fade-in" delay={200}>
        <footer className="py-10 shadow-inner transition-colors duration-1000 animated-gradient-section text-white [&_p]:text-white">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div
              className="grid grid-cols-2 md:grid-cols-4 gap-8 border-b border-opacity-30 pb-8 mb-8"
              ...
              <div className="col-span-2 md:col-span-1">
                ...
                <p className="mb-4 text-white">
                  A sua solução definitiva para gestão e facturação simplificada. Rápido, seguro e compatível com as normas fiscais.
                </p>
              ...
              <div>
                <h4 className="text-lg font-semibold mb-4" style={{ color: colors.secondary }}>
                  Navegação
                </h4>
                <ul className="space-y-2 text-sm">
                  {navLinks.map((link) => (
                    <li key={link.id}>
                      <button onClick={() => scrollToSection(link.id)} className="cursor-pointer transition hover:text-white text-white">
                        {link.name}
                      </button>
                    </li>
                  ))}
              ...
              <div>
                ...
                <ul className="space-y-2 text-sm">
                  <li>
                    <button onClick={() => scrollToSection("faq")} className="flex items-center gap-2 text-white">
                      <HelpCircle size={18} /> FAQ
                    </button>
                  </li>
                  <li>
                    <button className="flex items-center gap-2 text-white">
                      <Headset size={18} /> Suporte Técnico
                    </button>
                  </li>
                  <li>
                    <button className="flex items-center gap-2 text-white">
                  ...
                  <li>
                    <button className="flex items-center gap-2 text-white">
                      <BookOpenCheck size={18} /> Livro de Reclamações
                    </button>
                  </li>
                </ul>
              </div>
